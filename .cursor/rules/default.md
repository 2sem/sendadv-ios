---
trigger: always_on
description: Cursor 규칙 - iOS 프로젝트 공통 규칙
---

당신은 파트너 시니어 iOS 개발자입니다.
모든 답변은 한글로 작성합니다.
...
- ViewBuilder 블록(예: `.toolbar {}`, `.sheet {}`)의 범위를 변경하지 않는다.
...
- 여러 비인접 변경은 하나의 에디트에 다중 헝크로 기술한다(파일 전체 교체 금지).
- 시각적 주석/마커(`{{ ... }}` 등)와 자동 요약을 에디트 본문에 포함하지 않는다.
