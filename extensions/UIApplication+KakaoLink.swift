...
extension UIApplication{
    func shareByKakao(){
        ...
        let kakaoContent = KMTContentObject(title: UIApplication.shared.displayName ?? "", imageURL: URL(string: "https://is1-ssl.mzstatic.com/image/thumb/Purple118/v4/4e/e1/6d/4ee16d7c-64b1-f532-e95e-f335a5d9d4ea/mzl.hpndwfnb.png/150x150bb.jpg")!, link: kakaoLink);
        kakaoContent.imageWidth = 120;
        kakaoContent.imageHeight = 120; //160
        kakaoContent.desc = "특정 회사/부서/직위로 SMS를 보내고 싶다면?";
        ...
        let kakaoTemplate = KMTFeedTemplate.init(builderBlock: { (kakaoBuilder) in
            ...
            kakaoBuilder.addButton(KMTButtonObject(builderBlock: { (buttonBuilder) in
                ...
                buttonBuilder.title = "앱 스토어";
            }));
            
            /*kakaoBuilder.addButton(KLKButtonObject(builderBlock: { (buttonBuilder) in
                buttonBuilder.link = KLKLinkObject(builderBlock: { (linkBuilder) in
                    linkBuilder.webURL = URL(string: "https://play.google.com/store/apps/details?id=com.tarkarn.gbs&hl=ko");
                    linkBuilder.mobileWebURL = URL(string: "https://play.google.com/store/apps/details?id=com.tarkarn.gbs&hl=ko");
                })
                buttonBuilder.title = "구글플레이";
            }));*/
